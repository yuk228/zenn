...
sequenceDiagram
    ...
    App-->>Browser: 307 Redirect + Set-Cookie<br/>Location: discord.com/oauth2/authorize?state=xxx...

    Browser->>Discord: Discord認証画面に遷移
    Discord->>Browser: ユーザーが認可

    Discord-->>Browser: 302 Redirect<br/>Location: /api/callback?code=yyy&state=xxx

    Browser->>App: GET /api/callback?code=yyy&state=xxx
    Note over App: sessionのstateとURLの?stateを照合

    alt state 不一致 or 欠落
        App-->>Browser: 307 Redirect /verify/error
    else state 一致
        Note over App: session.code = yyy<br/>session.state = undefined
        App-->>Browser: 307 Redirect /verify + Set-Cookie
    end

    Browser->>App: GET /verify (Cookie)
    App-->>Browser: アプリ認証画面の表示
    Browser->>Browser: Turnstile を解く

    Browser->>App: POST /api/verify (Cookie, token)
