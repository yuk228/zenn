...
sequenceDiagram
    ...
    participant Client as Client(Next.js App)
    participant AS as Authorization Server(Discord)
    participant RS as Resource Server(Discord API)

    User->>Client: ログイン開始
    Client-->>User: Discord認証画面へ遷移(state付き)
    User->>AS: Discord認証画面に遷移
    AS->>User: ユーザーが認可
    AS-->>User: コールバックへ遷移(code, state付き)
    User->>Client: コールバック受信

    alt state 不一致
        Client-->>User: エラー画面に遷移
    else state 一致
        Client-->>User: アプリ検証画面に遷移
        User->>User: Turnstileを解く
        User->>Client: 検証結果を送信
        Client->>AS: code を提示しaccess_tokenを要求
        AS->>Client: access_token
        Client->>RS: access_tokenでユーザー情報要求 + ロール付与
        RS->>Client: ユーザー情報
        Client-->>User: 認証完了画面に遷移
    end
```
