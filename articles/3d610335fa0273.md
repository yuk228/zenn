...
sequenceDiagram
    ...
    Note over App: session.state と URL の state を照合
    ...
    App->>App: session.code を取り出す
    App->>Discord: code を access_token に交換
    Discord-->>App: access_token
    App->>Discord: ユーザー情報取得 / ロール付与
    App->>Discord: webhook 通知
